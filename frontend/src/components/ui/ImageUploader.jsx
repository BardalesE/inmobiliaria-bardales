  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ...
        style={{
          border: `2px dashed ${dropActive ? 'rgba(201,179,126,0.65)' : 'rgba(255,255,255,0.09)'}`,
          background: dropActive ? 'rgba(201,179,126,0.06)' : 'rgba(28,19,8,0.60)',
          padding: '30px 20px',
      ...
      >
        {dropActive && (
          <div className="absolute inset-0 pointer-events-none" style={{
            background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(201,179,126,0.09), transparent)',
          }} />
        )}
        <div className="relative flex flex-col items-center gap-2.5">
          <div
            ...
            style={{
              background: dropActive ? 'rgba(201,179,126,0.16)' : 'rgba(42,30,16,0.85)',
              border: `1px solid ${dropActive ? 'rgba(201,179,126,0.38)' : 'rgba(255,255,255,0.06)'}`,
              transition: 'all 0.18s ease',
            }}
          >
            <svg width="21" height="21" viewBox="0 0 24 24" fill="none"
              stroke={dropActive ? '#D8C48D' : '#9A8268'} strokeWidth="1.8"
          ...
          <div className="text-center">
            <p className="text-sm font-semibold" style={{
              color: dropActive ? '#D8C48D' : '#9A8268',
...
function ImageCard({ item, idx, isDragging, isDragOver, onDragStart, onDragOver, onDrop, onDragEnd, onRemove, onMakeCover, onRetry, onLightbox }) {
  ...
  return (
    <div
      ...
      style={{
        border: isDragOver
          ? '2px dashed rgba(201,179,126,0.65)'
          : '1px solid rgba(255,255,255,0.07)',
        background: '#1C1308',
        opacity: isDragging ? 0.40 : 1,
        cursor: 'grab',
        transition: 'opacity 0.15s ease, border-color 0.15s ease',
        boxShadow: isDragOver ? '0 0 0 4px rgba(201,179,126,0.12)' : 'none',
      }}
    >
      {/* Thumbnail */}
      ...
      {idx === 0 && item.status === 'done' && (
        <div
          ...
          style={{
            background: 'rgba(201,179,126,0.92)',
      ...
      {item.status === 'uploading' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2"
          ...
          <svg className="animate-spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#D8C48D" strokeWidth="2.5">
            <path d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeOpacity="0.22"/>
            <path d="M21 12a9 9 0 00-9-9" strokeLinecap="round"/>
          </svg>
          <div style={{ width: '68%', height: 3, background: 'rgba(255,255,255,0.10)', borderRadius: 99, overflow: 'hidden' }}>
            <div style={{
              ...
              background: 'linear-gradient(90deg, #C9B37E, #D8C48D)',
              width: `${item.progress}%`,
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2"
          ...
          {item.file && (
            <button
              ...
              style={{
                fontSize: 10, color: '#D8C48D', fontWeight: 700,
        ...
        >
          {idx > 0 && (
            <button
              ...
              style={{ background: 'rgba(201,179,126,0.88)', border: 'none', cursor: 'pointer' }}
            >

  ...
  return (
    <Link href={`/properties/${property.id}`} className="group block">
      <div className="bg-bark-800 border border-terra/20 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 group-hover:-translate-y-1.5 group-hover:shadow-2xl group-hover:border-terra/60">
        ...
        <div className="p-4">
          <p className="text-[10px] uppercase tracking-[0.14em] text-terra-light font-semibold mb-1">{property.ref}</p>
          <h3 className="font-display text-lg tracking-wide text-sand leading-tight mb-1.5">{property.title}</h3>
          <p className="flex items-center gap-1.5 text-[11px] text-sand-muted mb-3">
            📍 {property.address}, {property.district}
          </p>
          <div className="flex flex-wrap gap-1.5 mb-3">
            <span className="px-2 py-0.5 bg-bark-700 border border-white/5 rounded text-[10px] font-semibold text-sand">📐 {property.area} m²</span>
            {property.block && <span className="px-2 py-0.5 bg-bark-700 border border-white/5 rounded text-[10px] font-semibold text-sand">Mz {property.block}</span>}
            {property.lot && <span className="px-2 py-0.5 bg-bark-700 border border-white/5 rounded text-[10px] font-semibold text-sand">Lt {property.lot}</span>}
          </div>

      ...
      <section className="max-w-lg mx-auto px-4 py-16">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-4xl sm:text-5xl tracking-wider mb-3"
            style={{ background: 'linear-gradient(135deg,#FDF6E9 40%,#D9BC7A 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          ...
          <div className="flex items-center justify-center gap-2 mb-8">
            {STEPS.slice(0, 2).map((label, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className={`flex items-center gap-1.5 text-xs font-bold transition-all ${step === i ? 'text-terra-light' : step > i ? 'text-terra-light' : 'text-sand-muted'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold border transition-all ${
                    step > i ? 'bg-terra border-terra text-bark-900' :
                ...
                {i < 1 && <div className={`w-8 h-px ${step > i ? 'bg-terra' : 'bg-white/10'}`} />}
              </div>
            ))}
          </div>
        )}
        ...
        <div className="bg-bark-800 border border-terra/20 rounded-2xl p-6 shadow-xl">
          ...
          {step === 0 && (
            <div className="space-y-5">
              ...
              <button
                onClick={() => setStep(1)}
                disabled={!canNext0}
                className="w-full py-3 rounded-xl bg-gradient-to-br from-terra to-terra-light text-bark-900 font-bold tracking-wide shadow-lg shadow-terra/30 hover:shadow-terra/50 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Siguiente →
              </button>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-[2] py-3 rounded-xl bg-gradient-to-br from-terra to-terra-light text-bark-900 font-bold tracking-wide shadow-lg shadow-terra/30 hover:shadow-terra/50 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
          ...
          {step === 2 && done && (
            <div className="text-center py-6">
              <div className="w-16 h-16 rounded-full bg-terra/20 border border-terra/40 flex items-center justify-center text-3xl mx-auto mb-5">
                ✓
              </div>
              ...
              <div className="flex flex-col gap-3">
                <a
                  ...
                  className="w-full py-3 rounded-xl bg-gradient-to-br from-[#C9A44E] to-[#8F7332] text-bark-900 font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-terra/30"
                >

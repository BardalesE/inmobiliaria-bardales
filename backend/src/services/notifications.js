  ...
  const html = `
    <div style="font-family:sans-serif;max-width:500px;margin:auto;background:#1C1308;color:#FDF6E9;padding:24px;border-radius:12px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Nombre</td><td style="padding:8px 0;font-weight:bold;">${name}</td></tr>
        <tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Teléfono</td><td style="padding:8px 0;"><a href="tel:${phone}" style="color:#E07840;">${phone}</a></td></tr>
        ${email ? `<tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Email</td><td style="padding:8px 0;">${email}</td></tr>` : ''}
        ${propertyRef ? `<tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Propiedad</td><td style="padding:8px 0;color:#E07840;">${propertyRef} — ${propertyTitle || ''}</td></tr>` : ''}
        ${message ? `<tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Mensaje</td><td style="padding:8px 0;">${message}</td></tr>` : ''}
      </table>
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:500px;margin:auto;background:#1C1308;color:#FDF6E9;padding:24px;border-radius:12px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Teléfono</td><td style="padding:8px 0;"><a href="tel:${phone}" style="color:#E07840;">${phone}</a></td></tr>
        ${email ? `<tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Email</td><td style="padding:8px 0;">${email}</td></tr>` : ''}
        <tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Ciudad</td><td style="padding:8px 0;">${city}</td></tr>
        <tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Tipo de propiedad</td><td style="padding:8px 0;">${propertyType}</td></tr>
        ${description ? `<tr><td style="padding:8px 0;color:#9A8268;font-size:13px;">Descripción</td><td style="padding:8px 0;">${description}</td></tr>` : ''}
      </table>
      <div style="margin-top:20px;padding-top:16px;border-top:1px solid #352615;display:flex;gap:12px;flex-wrap:wrap;">
        ...
        ${email ? `<a href="mailto:${email}" style="display:inline-block;background:#C4622D;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:14px;">✉️ Email</a>` : ''}
      </div>
    </div>
  `
  await sendEmail(subject, html)

      ...
      >
        <div className="flex flex-col items-center gap-2.5 select-none">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center"
            ...
            {mode === 'video' ? <IconVideo color={dropActive ? '#E07840' : '#9A8268'} /> : <IconPdf color={dropActive ? '#E07840' : '#F87171'} />}
          </div>
            ...
            <div key={item.id} className="rounded-xl border border-white/8 bg-bark-700 overflow-hidden">
              ...
              <div className="flex items-center gap-3 p-3">
                {item.status === 'uploading' && (
                  <svg className="animate-spin flex-shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#E07840" strokeWidth="2.5">
                  ...
                  {item.status === 'uploading' && (
                    <div className="h-1 rounded-full overflow-hidden mt-1.5" style={{ background: 'rgba(255,255,255,0.08)' }}>
                      <div className="h-full rounded-full transition-all duration-300"
                        style={{ width: `${item.progress}%`, background: 'linear-gradient(90deg, #C4622D, #E07840)' }} />
                    </div>
                  )}
